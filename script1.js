...
//     const encodedFolder = encodeURI(folder);
//     try {
//         let a = await fetch(`http://127.0.0.1:5500/${encodedFolder}`);
//         if (!a.ok) {
//             throw new Error(`Failed to fetch folder: ${a.status} ${a.statusText}`);
//         }
...
//                 const songName = element.href.split(`/${folder}`)[1]; // Keep the raw file name (e.g., 00%20AM%20%20Gulsi.mp3)
//                 songsList.push(songName);
//             }
//         }
//         console.log(`Songs fetched from ${folder}:`, songsList);
//         return songsList;
//     } catch (error) {
...
//     const songUrl = `/${normalizedFolder}${track}`; // Use the raw track name (e.g., 00%20AM%20%20Gulsi.mp3)
//     console.log(`Attempting to play song: ${songUrl} (Folder: ${normalizedFolder}, Track: ${track})`);

//     // Pause and reset the current song if it's playing
...
//     document.querySelector(".songinfo").innerHTML = track.replace(/%20/g, " "); // Display with spaces
//     document.querySelector(".songtime").innerHTML = "00:00 / 00:00";
...
//         playMusic(firstSong, currentFolder); // Play the first song automatically
//         document.querySelector(".songinfo").innerHTML = firstSong.replace(/%20/g, " ");
//         document.querySelector(".songtime").innerHTML = `00:00 / 00:00`;
//         const audio = new Audio("/songs/" + firstSong);
//         audio.addEventListener("loadedmetadata", () => {
//             document.querySelector(".songtime").innerHTML = `00:00 / ${formatTime(audio.duration)}`;
//         });
//         // Update the play SVG for the first song in the song list
//         document.querySelectorAll(".playNow img").forEach(img => img.src = "play.svg");
...
//                     document.querySelector(".songtime").innerHTML = `00:00 / 00:00`;
//                     const audio = new Audio(`/${album.folder}` + selectedSong);
//                     audio.addEventListener("loadedmetadata", () => {
//                         document.querySelector(".songtime").innerHTML = `00:00 / ${formatTime(audio.duration)}`;
//                     });
//                     document.querySelectorAll(".playNow img").forEach(img => img.src = "play.svg");
...
//                 document.querySelector(".songinfo").innerHTML = firstSong.replace(/%20/g, " ");
//                 document.querySelector(".songtime").innerHTML = `00:00 / 00:00`;
//                 const audio = new Audio(`/${currentFolder}` + firstSong);
//                 audio.addEventListener("loadedmetadata", () => {
//                     document.querySelector(".songtime").innerHTML = `00:00 / ${formatTime(audio.duration)}`;
//                 });
//                 document.getElementById("play").src = "pause.svg";
...
//                 console.log(`Fetching songs from folder: ${album.folder}`);
//                 const songs = await getSongs(album.folder);
//                 console.log(`Songs for ${album.folder}:`, songs);
//                 songs.forEach(song => {
//                     allSongs.push({
//                         name: song.replace(/%20/g, " "), // Display name with spaces
//                         rawName: song, // Raw name for playback (e.g., 00%20AM%20%20Gulsi.mp3)
//                         folder: album.folder,
...
//         Array.from(songUL.getElementsByTagName("li")).forEach((e, idx) => {
//             e.addEventListener("click", () => {
//                 const selectedSong = filteredSongs[idx]?.rawName || songs[idx];
//                 const selectedFolder = filteredSongs[idx]?.folder || currentFolder;
...
//                     console.log(`Before playMusic - Song: ${selectedSong}, Folder: ${selectedFolder}`);
...
async function main() {
    ...
    if (songs.length > 0) {
        ...
        currentSong.src = `/${currentFolder}${firstSong}`;
        document.querySelector(".songinfo").innerHTML = firstSong.replace(/%20/g, " ");
        document.querySelector(".songtime").innerHTML = `00:00 / 00:00`;
        const audio = new Audio("/songs/" + firstSong);
        audio.addEventListener("loadedmetadata", () => {
